...
2.  **Obtener Connection Strings:**
    ...
    *   En la sección "Connection parameters", encontrarás los datos (Host, User, Password, Port, Database).
    *   Para Vercel (entorno serverless), es **CRÍTICO** usar el **Connection Pooler** para no agotar las conexiones.
    *   Busca la sección **Connection Pooling** (o Transaction Pooler).
    *   Copia la URL de conexión (debería usar el puerto `6543` y `pgbouncer` mode "Transaction").
...
```
*Nota: Usamos `--data-only` y `--exclude-table=django_migrations` porque es mejor correr primero las migraciones de Django (estructura) y luego importar solo los datos.*
...
2.  **Copiar los Datos:**
    ...
    ```
    *Excluimos `auth_permission` y `django_content_type` porque `migrate` ya los generó y suelen causar conflictos de duplicados.*
