...
import { ForecastData } from '../hooks/useForecasting';

interface BudgetRunwayCardProps {
    forecast: ForecastData | null;
    loading: boolean;
}

export function BudgetRunwayCard({ forecast, loading }: BudgetRunwayCardProps) {
  ...
  return (
    <Card variant="elevated" style={{ marginBottom: 16, borderLeftWidth: 4, borderLeftColor: getStatusColor() }}>
        ...
        <View style={{ flex: 1 }}>
          ...
          {forecast.forecast_date && (
             <View style={{ marginTop: 8, padding: 8, backgroundColor: colors.background, borderRadius: 8 }}>
                <Typography variant="caption" weight="medium" style={{ color: colors.textSecondary }}>
                   Fecha estimada de agotamiento:
                </Typography>
                <Typography variant="h3" weight="bold" style={{ color: getStatusColor() }}>
                   {forecast.forecast_date}
                </Typography>
             </View>
          )}
