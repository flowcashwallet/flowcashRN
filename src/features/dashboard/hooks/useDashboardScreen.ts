...
export const useDashboardScreen = () => {
  ...
  const formatWeeklyValue = useCallback((value: number) => {
  ...
  const expenseTrend = useMemo(() => {
    ...
    const hasData = totals.some((v) => v > 0);
    return { data, hasData };
  }, [currentMonthTransactions, formatWeeklyValue, periodView, selectedDate, year]);
  ...
  const categoryColor = useCallback(
    (category: string) => PIE_PALETTE[hashString(category) % PIE_PALETTE.length],
    [],
  );
  ...
  const onLineChartLayout = useCallback((width: number) => {
    setLineChartWidth(width);
  }, []);

  const onToggleWeeklyDetails = useCallback(() => {
  ...
  const onToggleExpandedWeek = useCallback((weekLabel: string) => {
